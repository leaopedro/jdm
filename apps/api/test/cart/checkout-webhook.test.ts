...
const seedCartWithTicketAndPickupProduct = async (userId: string) => {
  const event = await prisma.event.create({
    data: {
  ...
  const variant = await prisma.variant.create({
    data: {
      ...
      quantityTotal: 10,
      quantitySold: 1,
      attributes: { size: 'M', color: 'Branca' },
      active: true,
    },
  });
  const cart = await prisma.cart.create({
  ...
  return { cart, event, ticketOrder, productOrder };
};
...
describe('POST /stripe/webhook (cart checkout settlement)', () => {
  ...
  it('payment_intent.succeeded assigns pickup product orders to the paid event ticket', async () => {
    const { user } = await createUser({ verified: true });
    const { cart, event, ticketOrder, productOrder } = await seedCartWithTicketAndPickupProduct(
      user.id,
    );

    stripe.nextEvent = {
      id: 'evt_cart_pickup_assign',
      type: 'payment_intent.succeeded',
      data: {
        object: {
          id: 'pi_cart_pickup_assign',
          metadata: {
            cartId: cart.id,
            userId: user.id,
            orderIds: JSON.stringify([ticketOrder.id, productOrder.id]),
          },
        },
      },
    };
    ...
    const ticket = await prisma.ticket.findFirstOrThrow({
      where: { orderId: ticketOrder.id },
    });
    const settledProductOrder = await prisma.order.findUniqueOrThrow({
      where: { id: productOrder.id },
    });
    expect(settledProductOrder.status).toBe('paid');
    expect(settledProductOrder.fulfillmentStatus).toBe('pickup_ready');

    const note = JSON.parse(settledProductOrder.notes ?? '{}') as {
