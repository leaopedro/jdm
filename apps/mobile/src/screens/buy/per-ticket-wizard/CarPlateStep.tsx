...
function CarPlateStepScreen({ data, onNext, onBack }: WizardStepProps) {
  ...
  const [selectedCarId, setSelectedCarId] = useState<string | null>(
    (data.carId as string | undefined) ?? null,
  );
  const [plate, setPlate] = useState<string>((data.licensePlate as string | undefined) ?? '');
  ...
  const handleNext = () => {
    ...
    onNext({ carId: selectedCarId!, licensePlate: withDash, carLabel: label });
  };
  ...
  const canSubmit = selectedCarId !== null && plate.length >= 7;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        ...
        <Pressable
          ...
          accessibilityRole="button"
        >
          <Text style={styles.primaryLabel}>{buyCopy.carPlate.confirm}</Text>
        </Pressable>
      </View>
    </View>
  );
