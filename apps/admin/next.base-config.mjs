...
export function buildAdminNextConfig(env = process.env) {
  return {
    reactStrictMode: true,
    transpilePackages: ['@jdm/shared'],
    env: {
      NEXT_PUBLIC_API_BASE_URL: env.NEXT_PUBLIC_API_BASE_URL ?? 'http://localhost:4000',
    },
    allowedDevOrigins: parseAllowedDevOrigins(env.ALLOWED_DEV_ORIGINS),
...
export const adminNextConfig = buildAdminNextConfig();
